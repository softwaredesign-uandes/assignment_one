class BlockModel
  ...
  def reblock_no_fors(blocks, rx, ry, rz)
    blocks
      .group_by { |block| [(block[:x] / rx).to_i , (block[:y] / ry).to_i , (block[:z] / rz).to_i] }
      .map { |indices, group_blocks| { x: indices[0], y: indices[1], z: indices[2], mass: group_blocks.sum {|block| block[:mass] } } }
  end
